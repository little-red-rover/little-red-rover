                               ...
                               int32_t event_id, void *event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_AP_STACONNECTED) {
        ...
        if(
            ...
            wifi_prov_mgr_reset_provisioning();
            ESP_LOGI(TAG_STA, "Authentication failed. Resetting provisioning and restarting.");
            esp_restart();
        }
...
//    printf("webserver handle: %d \n", *(unsigned int *)_server_handle);
    ...
    if (!provisioned) {
	    ...
	    ESP_ERROR_CHECK( wifi_prov_mgr_start_provisioning(WIFI_PROV_SECURITY_1, NULL, "little_red_rover", NULL));
    } else {
